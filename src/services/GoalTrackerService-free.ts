...
const parseFeasibilityScore = (content: string): number => {
    const match = content.match(/feasibility(?:\s+score)?:?\s*(\d+)/i);
    return match ? parseInt(match[1]) : 50; // Default to 50 if not found
};

const parseSafeWithdrawalAnalysis = (content: string): string => {
    const match = content.match(/Safe Withdrawal (?:Rate )?Analysis:(.*?)(?:\n\n|$)/s);
    return match ? match[1].trim() : 'Standard 4% rule recommended';
};

const parseFireTarget = (content: string, type: 'lean' | 'traditional' | 'fat'): number => {
    const regex = new RegExp(`${type} fire target:?\\s*\\$?(\\d+(?:,\\d{3})*(?:\\.\\d{2})?)`,'i');
...
const getGoalAdviceFromFreeAI = async (savingsData: SavingsData): Promise<ChatGPTResponse> => {
    const createPrompt = (savingsData: SavingsData) => {
        ...
        if (isFIREGoal) {
            return `Analyze this FIRE goal: Current net worth $${savingsData.amount}, target $${savingsData.maturityAmount}, monthly income $${additionalContext.monthlyIncome}, expenses $${additionalContext.monthlyExpenses}. Provide specific advice, risk assessment, and quarterly milestones.`;
        } else {
            return `Analyze this savings goal: ${savingsData.savingName}, current $${savingsData.amount}, target $${savingsData.maturityAmount}, timeline ${new Date(Array.isArray(savingsData.endDate) ? savingsData.endDate[0] : savingsData.endDate).toLocaleDateString()}. Provide actionable advice and milestones.`;
        }
    };
